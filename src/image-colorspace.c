/*
 * "$Id: image-colorspace.c,v 1.5 2007-11-21 06:45:24 jpzhang Exp $"
 *
...
static clut_t	*ImageMatrix;	/* Color transform matrix LUT */
static PRINTER_CSPACE	ImageColorSpace = CSPACE_RGB;
...
void
ImageSetColorSpace(PRINTER_CSPACE cs)	/* I - Destination colorspace */
{
 /*
                ...
                float matrix[3][3])	/* I - Color transform matrix */
{
  ...
  */
  ...
  if (ImageMatrix == NULL)
    ImageMatrix = (clut_t*)calloc(3, sizeof(clut_t));

  if (ImageMatrix == NULL)
    return;

  if (ImageDensity == NULL)
    ImageDensity = (int*)calloc(256, sizeof(int));
  ...
  */
  ...
  for (k = 0, im = ImageDensity; k < 256; k ++)
    *im++ = (int)(255.0 * d * pow((float)k / 255.0, g) + 0.5);
}
  ...
  else
  {
    while (count > 0)
    {
      ...
      *out++ = *in++;
      ...
      if (ImageColorSpace >= CSPACE_CIELab)
        rgb_to_lab(out - 3);
      else if (ImageColorSpace == CSPACE_CIEXYZ)
        rgb_to_xyz(out - 3);
              ...
              int        count)	/* I - Number of pixels */
{
  ...
  else
  {
    ...
    if (ImageColorSpace >= CSPACE_CIEXYZ)
    {
      while (count > 0)
      {
	if (ImageColorSpace >= CSPACE_CIELab)
               ...
               int  hue)		/* I - Color hue (degrees) */
{
  ...
  static clut_t	*lut = NULL;	/* Lookup table for matrix */
      ...
      hue != last_hue)
  {
    ...
    */
    ...
    if (lut == NULL)
      lut = (clut_t*)calloc(3, sizeof(clut_t));
...
/*
 * End of "$Id: image-colorspace.c,v 1.5 2007-11-21 06:45:24 jpzhang Exp $".
 */
